...
public final class FSArray<T extends FeatureStructure> extends TOP
  ...
  public FSArray(JCas jcas, int length) {
    ...
    if (_casView.backend() != null) {
      _casView.backend().arrays().initializeFsArray(_id, length);
    }
  }
  ...
  public <U extends FeatureStructure> U get(int i) {
    if (_casView.backend() != null) {
      int targetFsRef = _casView.backend().arrays().getFsArrayRefValue(_id, i);
      TOP v = (targetFsRef == 0) ? null : _casView.getFsFromId_checked(targetFsRef);
      return (U) _maybeGetPearFs(v);
    }
  ...
  public void set(int i, T av) {
    ...
    if (_casView.backend() != null) {
      TOP base = _maybeGetBaseForPearFs(v);
      _casView.backend().arrays().setFsArrayRefValue(_id, i, (base == null) ? 0 : base._id());
    } else {
  ...
  void set_without_PEAR_conversion(int i, TOP v) {
    if (_casView.backend() != null) {
      _casView.backend().arrays().setFsArrayRefValue(_id, i, (v == null) ? 0 : v._id());
    } else {
      theArray[i] = v;
    }
    _casView.maybeLogArrayUpdate(this, null, i);
  ...
  public int size() {
    if (_casView.backend() != null) {
      return _casView.backend().arrays().sizeFsArray(_id);
    }
  ...
  public <U extends FeatureStructure> void copyFromArray(U[] src, int srcPos, int destPos,
    ...
    if (_casView.backend() != null) {
      ...
      for (int i = 0; i < length; i++) {
        TOP v = (TOP) src[srcPos + i];
        if (v != null && _casView.getBaseCAS() != v._casView.getBaseCAS()) {
          throw new CASRuntimeException(CASRuntimeException.FS_NOT_MEMBER_OF_CAS, v, v._casView,
                  _casView);
        }
        TOP base = _maybeGetBaseForPearFs(v);
        refs[i] = (base == null) ? 0 : base._id();
      }
      _casView.backend().arrays().copyFromFsArray(_id, destPos, refs, 0, length);
  ...
  public <U extends FeatureStructure> void copyToArray(int srcPos, U[] dest, int destPos,
    ...
    if (_casView.backend() != null) {
      int[] refs = new int[length];
      _casView.backend().arrays().copyToFsArray(_id, srcPos, refs, 0, length);
      for (int i = 0; i < length; i++) {
        int ref = refs[i];
        TOP v = (ref == 0) ? null : _casView.getFsFromId_checked(ref);
        dest[destPos + i] = (U) _maybeGetPearFs(v);
      }
    } else {
      while (srcPos < srcEnd && destPos < destEnd) {
  ...
  public void copyToArray(int srcPos, String[] dest, int destPos, int length) {
    ...
    for (int i = 0; i < length; i++) {
      FeatureStructure fs = _maybeGetPearFs(get_without_PEAR_conversion(i + srcPos));
  ...
  public TOP[] _getTheArray() {
    if (_casView.backend() != null) {
      ...
      TOP[] r = new TOP[sz];
      for (int i = 0; i < sz; i++) {
        r[i] = get_without_PEAR_conversion(i);
      }
      return r;
    }
    return theArray;
  ...
  public void copyValuesFrom(CommonArrayFS<T> v) {
    ...
    int sz = size();
    for (int i = 0; i < sz; i++) {
      set_without_PEAR_conversion(i, bv.get_without_PEAR_conversion(i));
    }
    _casView.maybeLogArrayUpdates(this, 0, sz);
  }
