...
public final class FloatArray extends TOP
  ...
  public FloatArray(JCas jcas, int length) {
    ...
    if (_casView.backend() != null) {
      _casView.backend().arrays().initializeFloatArray(_id, length);
    }
  }
  ...
  public float get(int i) {
    if (_casView.backend() != null) {
      return _casView.backend().arrays().getFloatArrayValue(_id, i);
    }
    return theArray[i];
  }
  ...
  public void set(int i, float v) {
    if (_casView.backend() != null) {
      _casView.backend().arrays().setFloatArrayValue(_id, i, v);
    } else {
      theArray[i] = v;
    }
  ...
  public void copyFromArray(float[] src, int srcPos, int destPos, int length) {
    if (_casView.backend() != null) {
      _casView.backend().arrays().copyFromFloatArray(_id, destPos, src, srcPos, length);
    } else {
  ...
  public void copyToArray(int srcPos, float[] dest, int destPos, int length) {
    if (_casView.backend() != null) {
      _casView.backend().arrays().copyToFloatArray(_id, srcPos, dest, destPos, length);
    } else {
  ...
  public int size() {
    if (_casView.backend() != null) {
      return _casView.backend().arrays().sizeFloatArray(_id);
    }
  ...
  public void copyToArray(int srcOffset, String[] dest, int destOffset, int length) {
    _casView.checkArrayBounds(size(), srcOffset, length);
    for (int i = 0; i < length; i++) {
      dest[i + destOffset] = Float.toString(get(i + srcOffset));
    }
  }
  ...
  public void copyFromArray(String[] src, int srcOffset, int destOffset, int length) {
    _casView.checkArrayBounds(size(), destOffset, length);
    for (int i = 0; i < length; i++) {
