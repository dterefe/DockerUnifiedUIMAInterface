...
public final class AppMetrics implements AutoCloseable {
    ...
    private void record(String metric, java.util.Map<String,String> labels, double value) {
        String key = keyOf(metric, labels);
        series.computeIfAbsent(key, k -> new java.util.ArrayList<>()).add(value);
    }


    public AppMetrics(String runName) {
    ...
    private void sampleSafely() {
        try {
            ...
            prevTicks = ticks;
            record("host_cpu_load_ratio", null, sysLoad);

            // CPU Prozess
            OSProcess current = os.getProcess(os.getProcessId());
            double procLoad = current.getProcessCpuLoadBetweenTicks(prevProc);
            prevProc = current;
            record("process_cpu_load_ratio", null, procLoad);

            // Memory
            record("host_memory_used_bytes", null, mem.getTotal() - mem.getAvailable());
            record("process_resident_memory_bytes", null, current.getResidentSetSize());
            ...
            for (Thread.State st : Thread.State.values()) {
                record("jvm_threads_state", java.util.Map.of("state", st.name()), cnt.getOrDefault(st, 0));
            }
    ...
    private void writeSnapshotJson() throws Exception {
        ...
        Path out = Path.of("/home/dater/projects/duui-load-simulator/runs/" + runName + ".json");
