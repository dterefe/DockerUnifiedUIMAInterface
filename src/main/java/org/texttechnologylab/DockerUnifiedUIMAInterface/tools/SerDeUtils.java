...
public class SerDeUtils {
    ...
    public static String inferMimeTypeFromPathOrDefault(DUUIDocument document) {
        return Mime.inferMimeTypeFromBestSourceOrEmpty(document);
    }
    ...
    static final class Mime {
        ...
        private static String bestNameForGuessOrEmpty(DUUIDocument document) {
            ...
            String ext = document.getFileExtension();
            String normalizedExt = normalizeExtension(ext);
            if (!normalizedExt.isBlank()) {
                // Provide a synthetic name that still matches suffix-based rules.
                return "file" + normalizedExt;
            }

            return "";
        }

        static String inferMimeTypeFromBestSourceOrEmpty(DUUIDocument document) {
            String best = bestNameForGuessOrEmpty(document);
            if (best.isBlank()) return "";
            return guessMimeTypeFromNameOrEmpty(best);
        }
        ...
        static void ensureCanonicalMimeType(DUUIDocument document) {
            ...
            String inferred = inferMimeTypeFromBestSourceOrEmpty(document);
            if (!inferred.isBlank()) {
            ...
            try {
                ...
                if (bytes != null && bytes.length > 0) {
                    String sniffed = guessMimeTypeFromStreamOrEmpty(new ByteArrayInputStream(bytes));
                    if (!sniffed.isBlank()) {
        ...
        static String guessMimeTypeFromNameOrEmpty(String nameOrPath) {
        ...
        static String guessMimeTypeFromStreamOrEmpty(InputStream in) throws IOException {
            if (in == null) return "";
